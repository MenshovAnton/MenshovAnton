<h1 align="center">Hi there, I'm Anton 
<img src="https://github.com/blackcater/blackcater/raw/main/images/Hi.gif" height="32"/></h1>
<h3 align="center">Computer science student, cybersecurity specialist from Russia 🇷🇺</h3>

### 🛠️ Tech & Tools
- **Core Languages:** C# | Java
- **Game Engine & Frameworks:** Unity
- **Design & Media:** Blender | Photoshop | DaVinci Resolve
- **Systems & Tools:** Windows, Linux, Bash, SSH | Powered by JetBrains IDEs

### 🌟 Key Highlights
- **3rd Place** at the regional "Professionals" Championship in **Mobile Game Development**.
- **Prize Winner** at "Big Challenges" competition with [VoiceAssistantLeo](https://github.com/MenshovAnton/VoiceAssistantLeo).

### ✨ GitHub Stats
<div align="center">
    <img src="http://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=MenshovAnton&theme=github_dark"/>
    <img src="http://github-profile-summary-cards.vercel.app/api/cards/stats?username=MenshovAnton&theme=github_dark"/>
    <img src="http://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=MenshovAnton&theme=github_dark&exclude=markdown"/>
</div>

<h1>На чай</h1>
<a href="https://www.donationalerts.com/r/waysoon"><img alt="Static Badge" src="https://img.shields.io/badge/DonationAlerts-Waysoon-red?style=for-the-badge"></a>
